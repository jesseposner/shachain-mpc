...
seed, exists. What remains online is one round of input-label exchange plus
local evaluation.
...
|---|---:|---:|---:|---:|---:|
| K=1 edge | 10 ms | 8.7 KB | 1 | 0.26 s | 34 MB |
| K=10 edges | 100 ms | 8.7 KB | 1 | 2.4 s | 336 MB |
| K=48 cold start | 0.49 s | 8.7 KB | 1 | 11.4 s | 1.61 GB |
| K=48 cold start, semi-honest | 0.54 s | 8.7 KB | 1 | 3.8 s | 659 MB |
...
1. With pre-garbled circuits, channel open costs one round trip plus ~0.5 s of
   local compute, against ~77,000 sequential rounds for Rep3. Over any real
   network this is the difference between sub-second and minutes.
...
- Channel open: pre-garbled 48-edge BMR package per expected channel,
  evaluated in one round trip at open time; the package is consumed once and
  its storage freed.
...
- A WAN run to confirm the single-round online phase behaves as computed.
