...
**Idea.** Rounds equal a circuit's AND-depth, and the Bristol sha256 circuit
is 1,607 deep, almost all of it addition, because it uses ripple-carry
adders at roughly 31 deep per 32-bit add. MP-SPDZ's own `sbitint` addition
looked far shallower when measured on its own:
...
**Result.** 2,456 rounds. Half again worse than the circuit it was meant to
replace. The program also returns a wrong digest, and that bug was not chased
because the performance case had already failed.

**Why the extrapolation was wrong.** A chain of bare additions is the best
case for the compiler: nothing else competes and the additions merge. In
...
**What would actually be needed.** Carry-save adder trees, so that the
...
**The cheaper route to the same goal.** Garbling is input-independent, so any
circuit the system will need can be garbled ahead of time and evaluated in
